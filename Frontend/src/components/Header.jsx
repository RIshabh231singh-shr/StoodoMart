...
import { ShoppingCart, Search, User, Package, LogOut, ChevronDown, Menu, X, ShieldAlert, Building2, Bell } from 'lucide-react';
    ...
    >
      <div className="container mx-auto px-2 sm:px-4 md:px-8">
        <div className="flex items-center justify-between">
          ...
          {/* Logo & Brand */}
          <Link to="/" className="flex items-center gap-1 sm:gap-3 z-50">
            <img src={logo} alt="StoodoMart Logo" className="w-10 sm:w-16 h-auto" />
            <h1 className={`text-xl sm:text-2xl font-extrabold tracking-tight m-0 ${isScrolled ? 'text-slate-800' : 'text-slate-800'}`}>
          ...
          <div className="flex items-center gap-2 sm:gap-5">
            ...
            {/* Notification Icon */}
            {isAuthenticated && (
              <div className="relative group cursor-pointer" ref={notificationRef}>
                <div
                ...
                >
                  <Bell size={24} className="text-slate-700 group-hover:text-amber-500 transition-colors" />
                </div>
                {unreadCount > 0 && (
                ...
                {notificationsOpen && (
                  <div className="absolute right-0 mt-3 w-72 md:w-80 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 animate-slide-up transform origin-top-right z-50">
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {(!notifications || notifications.length === 0) ? (
                        ...
                        notifications.map((notif) => (
                          <div
                            key={notif.id}
                            className={`px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer ${!notif.read ? 'bg-amber-50/50' : ''}`}
                            onClick={() => {
          ...
          <div className="flex flex-col gap-3 mt-4">
            {isAuthenticated ? (
              <>
                ...
                <Link to="/my-orders" className="flex items-center gap-3 py-2 text-slate-700"><Package size={18} /> My Orders</Link>
                {user?.role === 'Admin' && (
                  <>
                    <Link to="/add-product" className="flex items-center gap-3 py-2 text-slate-700"><Package size={18} /> Add Product</Link>
                    <Link to="/my-products" className="flex items-center gap-3 py-2 text-slate-700"><Package size={18} /> My Products</Link>
                    <Link to="/all-orders" className="flex items-center gap-3 py-2 text-slate-700"><Package size={18} /> All Orders</Link>
                  </>
                )}
                {user?.role === 'SuperAdmin' && (
