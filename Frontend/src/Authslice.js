...
export const RegisterUser = createAsyncThunk("auth/register", async (registerData, { rejectWithValue }) => {
    try {
        ...
        // We do NOT set user or isAuthenticated here because they need OTP verification
        return response.data;
    } catch (error) {
...
export const VerifyOtpThunk = createAsyncThunk("auth/verifyOtp", async ({ email, otp }, { rejectWithValue }) => {
    try {
        const response = await axiosClient.post("/person/verify-otp", { email, otp });
        return response.data;
    } catch (error) {
...
export const ChangePasswordThunk = createAsyncThunk("auth/changePassword", async (passwordData, { rejectWithValue }) => {
    try {
        const response = await axiosClient.patch("/person/change-password", passwordData);
        return response.data;
    } catch (error) {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(VerifyOtpThunk.fulfilled, (state, action) => {
                state.loading = false;
                state.user = action.payload.person;
                state.isAuthenticated = true;   // ← authenticated HERE, after OTP
            })
