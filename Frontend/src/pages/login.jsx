...
export default function Login() {
  ...
  // Use useEffect to handle side effects after successful API call
  useEffect(() => {
    if (isAuthenticated) {
      navigate('/');
    }
  ...
  const onSubmit = (data) => {
    // Dispatch the Redux thunk with the valid form data
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-mesh-gradient animate-gradient-bg p-8 overflow-hidden relative before:content-[''] before:absolute before:rounded-full before:blur-[80px] before:z-0 before:opacity-60 before:w-[400px] before:h-[400px] before:bg-brand-teal before:-top-[100px] before:-left-[100px] before:animate-float after:content-[''] after:absolute after:rounded-full after:blur-[80px] after:z-0 after:opacity-60 after:w-[500px] after:h-[500px] after:bg-brand-red after:-bottom-[150px] after:-right-[100px] after:animate-float-reverse">
      <div className="relative z-10 bg-white/85 backdrop-blur-md border border-white/40 rounded-3xl p-8 sm:p-12 w-full max-w-[450px] shadow-2xl opacity-0 translate-y-[30px] animate-slide-up">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold text-[#1a1a2e] m-0 tracking-tight">Stoodo<span className="text-brand-red">Mart</span></h1>
        </div>
        
        <p className="text-center text-slate-500 text-[0.95rem] mb-6">Welcome back! Please enter your details.</p>

        {error && (
          <div className="mb-6 p-4 bg-red-100 text-red-700 rounded-xl text-sm font-medium border border-red-200">
            {typeof error === 'string' ? error : error?.message || 'Failed to login'}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="login-form">
          ...
          <div className="mb-6 relative group">
            <label htmlFor="password" className="block text-sm font-semibold text-slate-700 mb-2 transition-colors duration-300 group-focus-within:text-[#00B4DB]">Password</label>
            <div className="relative">
              <input 
                type={showPassword ? "text" : "password"} 
                id="password"
                className={`w-full px-5 py-3.5 text-base text-slate-800 bg-white/90 border-2 rounded-xl outline-none transition-all duration-300 placeholder:text-slate-400 focus:ring-4 focus:bg-white pr-12 ${
              ...
              <button 
                type="button"
                className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-brand-teal transition-colors focus:outline-none"
