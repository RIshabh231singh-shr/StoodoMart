...
export default function Privacy() {
  ...
  // Scroll Spy: auto-highlight sidebar as user scrolls through content
  useEffect(() => {
    const sectionIds = policySections.map((s) => s.id);
    const observers = [];
    ...
    sectionIds.forEach((id) => {
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          // Only update from scroll, not from programmatic click-scroll
          if (entry.isIntersecting && !isClickScrolling.current) {
            setActiveSection(id);
          }
        },
        {
          rootMargin: "-30% 0px -60% 0px", // trigger when section is in the top 30-40% of viewport
          threshold: 0,
        }
      );
  ...
  const scrollToSection = (id) => {
    ...
    if (element) {
      ...
      // Re-enable scroll spy after animation (~800ms)
                  ...
                  {policySections.map((section) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-200 ${isActive
            ...
            <article className="w-full md:w-3/4 lg:w-3/4 max-w-3xl bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-slate-200 prose prose-slate prose-headings:font-black prose-h2:text-3xl prose-h2:mb-6 prose-p:text-slate-600 prose-p:leading-relaxed prose-a:text-brand-teal pb-20">
              ...
              <section id="contact" className="scroll-mt-32">
                ...
                <div className="bg-slate-900 text-white p-6 rounded-2xl flex flex-col sm:flex-row gap-6 items-center mt-6">
                  ...
                  <Link to="/contact" className="px-6 py-2 bg-white text-slate-900 font-bold rounded-xl hover:bg-slate-100 transition-colors whitespace-nowrap">
                    Contact Support
                  </Link>
                </div>
              </section>
